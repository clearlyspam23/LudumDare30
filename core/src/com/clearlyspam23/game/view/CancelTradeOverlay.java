...
public class CancelTradeOverlay extends ModularOverlay {
	
	private Map<Button, TradeAgreement> tradesMap = new HashMap<Button, TradeAgreement>();

	public CancelTradeOverlay(OverlayStage stage, NinePatch background, Planet source, final PlanetOverlay pOverlay) {
		...
		for(TradeAgreement t : source.getActiveTrades()){
			...
			Image image = new Image(new TextureRegionDrawable(stage.resourceMap.get(t.resource)));
			image.setSize(64, 64);
			overlay.addActor(image);
			final Image acceptOverlay = new Image(new TextureRegionDrawable(stage.declineOverlay));
			...
			float calculatedValue = t.planet.getResourceBuyingValue(t.resource)*source.getSpacePort().getCapacity()/((source.getSpacePort().getRate() + source.distanceToPlanet(t.planet))*GameStage.TICK_RATE);
			Label label = new Label("Cancel " + t.resource.name + " to " + t.planet.getName(), stage.skin);
			label.setAlignment(Align.center);
			textTable.add(label);
			textTable.row();
			label = new Label("estimated loss: " + poorMansFormat(calculatedValue), stage.skin, "small");
			label.setAlignment(Align.center);
			textTable.add(label);
		...
		button.addListener(new ClickListener(){
			public void clicked(InputEvent event, float screenX, float screenY){
				List<TradeAgreement> trades = new ArrayList<TradeAgreement>();
				for(Entry<Button, TradeAgreement> e : tradesMap.entrySet()){
					if(e.getKey().isChecked())
						trades.add(e.getValue());
				}
				pOverlay.cancelTrades(trades);
				hide();
			}
		});
