...
public class MinePurchaseRender extends MineRender {
	
	private TextureRegionDrawable moneyDraw;
	private Image levelOverlay;
	private TextureRegionDrawable overlay;

	public MinePurchaseRender(Drawable background, float width,
			Map<Resource, TextureRegion> resourceMap,
			TextureRegion structureIcon, Mine mine, Skin skin, TextureRegion money, TextureRegionDrawable overlay) {
...
//	}
	...
	protected Actor getSubTitle(float width) {
		...
		final Label label = new Label(Integer.toString(mine.getBuyingValue()), skin);
		table.add(label);
		return table;
	}
